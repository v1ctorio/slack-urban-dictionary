...
slack.action("next", async ({ ack, action, body, respond, client, say}) => {
	...
	try {
		...
		await respond({replace_original: false,response_type:'ephemeral',text:'Error getting the next definition.'})
		return;
	}
});
...
slack.action("previous", async ({ ack, action, body, respond, client,say }) => {
	...
	try {
		...
		await respond({replace_original: false,response_type:'ephemeral',text:'Error getting the previous definition.'})
		return;
	}
});
